    ...
    <div className="w-full bg-gray-50 shadow-blue-100 shadow-md h-20 flex justify-between items-center px-4 py-2 sm:p-6 sm:px-24 absolute top-0">
...

      {
        ...
        <div className="w-8 h-8 flex justify-center font-semibold items-center rounded-full bg-blue-900 text-white relative group">
          ...
          <div className="absolute hidden group-hover:block w-36 top-8 right-0 z-10 text-amber-200 bg-black rounded p-2">
            <ul className="list-none m-0 p-2 text-sm">
              {!userData.isAccountVerified &&  
              <li onClick={sendVerificationOtp}
              className="py-1 px-2 hover:bg-gray-700 cursor-pointer">Verify email</li>}

              <li onClick={logout}
               className="py-1 px-2 hover:bg-gray-700 cursor-pointer pr-10">Logout</li>
            </ul>
          </div>
        </div> 
        :  // Login Button 
